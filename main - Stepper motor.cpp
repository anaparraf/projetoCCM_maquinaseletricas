...
bool dir = 1; // Variavel booleana que em 0 define um sentido de rotação do motor de passo e em 1 o outro sentido
char leitura; // Le o que foi escrito na porta serial
int espera = 150; // Define o tempo em ms entre as fases (vai de 0 a 9 sendo 0 a velocidade máxima e 9 a mínima)
void switch_fase() {
  if (onoff == 0) { // Se onoff for zero, ou seja, o botão do usuário não foi pressionado ou não foi escrito A na entrada serial ou foi escrito a na entrada serial, o motor de passo não liga ou desliga
  ...
  else if (onoff == 1) { // Se onoff for 1, ou seja, o botão do usuário foi pressionado ou A foi escrito na entrada serial, o motor de passo liga
    ...
    else if (dir == 0) { // Se dir for 0, o sentido de rotação do motor é  horário
        EN_A = 0;
        IN1_A = 1; IN2_A = 0;
        EN_B = 1;
        IN1_B = 0; IN2_B = 1;
...
int main() {
    ...
    while (1) {
        switch (leitura){
            ...
            case '9': espera = 5; break;// wait_ms (espera) = 10 ms (velocidade máxima)
            case '8': espera = 10; break;// wait_ms (espera) = 20 ms
            case '7': espera = 15; break;// wait_ms (espera) = 30 ms
            case '6': espera = 20; break;// wait_ms (espera) = 40 ms
            case '5': espera = 25; break;// wait_ms (espera) = 50 ms
            case '4': espera = 30; break;// wait_ms (espera) = 60 ms
            case '3': espera = 35; break;// wait_ms (espera) = 70 ms
            case '2': espera = 40; break;// wait_ms (espera) = 80 ms
            case '1': espera = 45; break;// wait_ms (espera) = 90 ms
            case '0': espera = 50; break;// wait_ms (espera) = 100 ms (velocidade mínima)
        }
